...
impl LuppoBackend {
        ...
        if let Ok(db) = open_luppo_db() {
            if let Ok(installed) = db.list_installed_packages() {
                for pkg in installed {
                    ...
                    info.homepage = pkg.homepage.clone().unwrap_or_default();
                    info.category = category;
                    info.icon_name = name.clone();
                    info.icon_path = icon_path;
                    info.installed = true;
                    info.installed_size = format_bytes(pkg.total_size);
                    info.developer = pkg.packager
                        .as_ref()
                        .map(|p| p.name.clone())
                        .filter(|n| !n.is_empty())
                        .unwrap_or_else(|| detect_developer(&name, &info.homepage, &info.component));
                ...
                if let Ok(available) = db.list_available_packages() {
                    for pkg in available {
                        ...
                        info.dependencies_count = pkg
                            ...
                            .unwrap_or(0);
                        info.developer = pkg.effective_packager()
                            .map(|p| p.name.clone())
                            .filter(|n| !n.is_empty())
                            .unwrap_or_else(|| detect_developer(&name, &info.homepage, &info.component));
                        if let Some(hp) = pkg.effective_homepage() {
                            if !hp.is_empty() { info.homepage = hp; }
                        }
        ...
        if self.luppo_available {
            if let Ok(db) = open_luppo_db() {
                if let Ok(Some(remote)) = db.get_available_package(pkg_name) {
                    ...
                    if let Some(hp) = remote.effective_homepage() {
                        if !hp.is_empty() { pkg.homepage = hp; }
                    }
                    if let Some(p) = remote.effective_packager() {
                        if !p.name.is_empty() { pkg.developer = p.name; }
                    }
                }
                if let Ok(Some(inst)) = db.get_installed_package(pkg_name) {
                    ...
                    if let Some(hp) = &inst.homepage {
                        if !hp.is_empty() { pkg.homepage = hp.clone(); }
                    }
                    if let Some(p) = &inst.packager {
                        if !p.name.is_empty() { pkg.developer = p.name.clone(); }
                    }
                }
            }
        }
        ...
        if let Ok(out) = output {
            if out.status.success() {
                ...
                for line in text.lines() {
                    ...
                    if let Some(avail) = self.available_packages.get(&key) {
                        if !avail.description.is_empty() {
                            pkg.description = avail.description.clone();
                        }
                        if !avail.summary.is_empty() {
                            pkg.summary = avail.summary.clone();
                        }
                    }
    ...
    pub fn load_available_flatpaks(&mut self) {
        ...
        let cache_path = dirs::cache_dir()
            ...
            .join("flatpak_available_cache.json");

        // Try loading from disk cache if it exists and is less than 6 hours old
                    ...
                    if let Ok(elapsed) = modified.elapsed() {
                        if elapsed.as_secs() < 6 * 3600 {
                            if let Ok(content) = std::fs::read_to_string(&cache_path) {
                                if let Ok(cached_pkgs) = serde_json::from_str::<Vec<PackageInfo>>(&content) {
                                    for mut pkg in cached_pkgs {
                                        let needs_icon_refresh = if pkg.icon_path.starts_with("file://") {
                                            let p = pkg.icon_path.trim_start_matches("file://");
                                            !Path::new(p).exists()
                                        } else {
                                            pkg.icon_path.is_empty()
                                        };
                                        if needs_icon_refresh {
                                            let app_id = pkg.name.strip_prefix("flatpak:").unwrap_or(&pkg.name);
                                            pkg.icon_path = find_flatpak_icon(app_id);
                                        }
        ...
        let output = Command::new("flatpak")
            .args(["remote-ls", "--columns=application,name,version,origin,download-size,installed-size"])
            .output();

        if let Ok(out) = output {
            if out.status.success() {
                let text = String::from_utf8_lossy(&out.stdout);
                let mut flatpaks_to_cache = Vec::new();
                for line in text.lines() {
                    let parts: Vec<&str> = line.split('\t').map(|s| s.trim()).collect();
                    if parts.is_empty() { continue; }
                    let app_id = parts[0];
                    ...
                    pkg.version = version.to_string();
                    pkg.summary = format!("{} · {}", origin, app_id);
                    pkg.description = format!("{}", display_name);
                    pkg.developer = extract_flatpak_developer(app_id);
                    pkg.category = cat;
                    pkg.icon_name = icon_name;
                    pkg.icon_path = icon_path;
                    ...
                    flatpaks_to_cache.push(pkg.clone());
                    self.available_packages.insert(key, pkg);
                }

                // Write disk cache for instant startup next time
                if let Some(parent) = cache_path.parent() {
                    let _ = std::fs::create_dir_all(parent);
                }
                if let Ok(json) = serde_json::to_string(&flatpaks_to_cache) {
        ...
        F: FnMut(ProgressEvent) + Send + 'static,
    {
        ...
        if pkg_name.starts_with("flatpak:") {
            ...
            return self.run_flatpak_with_progress(pkg_name, action, on_progress).await;
        }
        if !self.luppo_available {
            return (false, i18n::tr("luppo_missing"));
        }
        ...
        self.run_luppo_with_progress(&[action_cmd, "-y", pkg_name], action_name, pkg_name, on_progress).await
    }
    ...
    {
        if pkg_name.starts_with("flatpak:") {
            return self.run_flatpak_with_progress(pkg_name, "remove", on_progress).await;
        }
        if !self.luppo_available {
            return (false, i18n::tr("luppo_missing"));
        }
        self.run_luppo_with_progress(&["remove", "-y", pkg_name], "remove", pkg_name, on_progress).await
    }
        ...
        F: FnMut(ProgressEvent) + Send + 'static,
    {
        ...
        RUNNING_PIDS.lock().unwrap().insert(pkg_name.to_string(), child.id());
        ...
        let stdout_task = tokio::task::spawn_blocking(move || {
            read_output_lines(stdout, |line| {
                ...
                if let Some(e) = event {
                    if let Ok(mut cb) = on_progress.lock() { cb(e); }
                }
            });
        });
        ...
        let stderr_task = tokio::task::spawn_blocking(move || {
            read_output_lines(stderr, |line| {
                ...
                if let Some(e) = event {
                    if let Ok(mut cb) = on_progress_stderr.lock() { cb(e); }
                }
            });
        });
        ...
        let status = match child.wait() {
            Ok(s) => {
                RUNNING_PIDS.lock().unwrap().remove(pkg_name);
                s
            }
            Err(e) => {
                RUNNING_PIDS.lock().unwrap().remove(pkg_name);
...
fn to_file_url(p: String) -> String {
    if p.starts_with('/') {
        format!("file://{}", p)
    } else {
        p
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        for dir in &flatpak_dirs {
            for ext in ICON_EXTENSIONS {
                ...
                if p.exists() {
                    return to_file_url(p.to_string_lossy().to_string());
                }
            }
        }
    }
...
fn search_appstream_icons(app_id: &str) -> Option<String> {
    ...
    let sizes = ["128x128", "64x64", "scalable", "256x256", "512x512"];
    let exts = [".png", ".svg", ".xpm", ".jpg"];
            ...
            for repo in arch_entries.flatten() {
                ...
                if let Ok(sub_entries) = std::fs::read_dir(&repo_path) {
                    for arch in sub_entries.flatten() {
                        ...
                        // Priority 1: Check 'active' symlink directory first (always points to current hash)
                        let active_icons = arch_path.join("active").join("icons");
                        if active_icons.exists() {
                            for size in sizes {
                                let dir = active_icons.join(size);
                                for ext in exts {
                                    let file = dir.join(format!("{}{}", app_id, ext));
                                    if file.exists() {
...
pub fn find_flatpak_icon(app_id: &str) -> String {
    ...
    if let Some(appstream_icon) = search_appstream_icons(app_id) {
        return to_file_url(appstream_icon);
    }
    ...
    // Skip packages/runtimes/extensions that have no icon on Flathub CDN
    let no_cdn_icon = app_id.starts_with("org.gtk.")
        || app_id.starts_with("org.freedesktop.")
        || app_id.starts_with("org.flatpak.")
        || app_id.starts_with("org.flathub.")
        || app_id.ends_with(".BaseApp")
        || app_id.ends_with(".Debug")
        || app_id.ends_with(".Locale")
        || app_id.ends_with(".Sources");
    if no_cdn_icon {
        return "qrc:/qml/assets/icons/package-x-generic.svg".to_string();
    }
...
fn parse_float_lenient(s: &str) -> Option<f32> {
    let clean = s.trim_matches(|c: char| !c.is_ascii_digit() && c != '.' && c != ',')
        .replace(',', ".");
    clean.parse::<f32>().ok()
}

fn extract_percentage_or_ratio(line: &str) -> Option<u8> {
    // 1. Check for percentage tokens (e.g. 45%, %45, [45%], (45%), 45.2%, %45,2 etc.)
    let words: Vec<&str> = line.split_whitespace().collect();
    for (i, word) in words.iter().enumerate() {
        let clean = word.trim_matches(|c: char| c == '(' || c == ')' || c == '[' || c == ']' || c == '{' || c == '}' || c == ',' || c == ':');
        
        if clean.starts_with('%') {
            let num_part = clean.trim_start_matches('%').trim_end_matches(|c: char| !c.is_ascii_digit() && c != '.' && c != ',');
            if let Some(val) = parse_float_lenient(num_part) {
                return Some(val.round().clamp(0.0, 100.0) as u8);
            }
        } else if clean.ends_with('%') {
            let num_part = clean.trim_end_matches('%').trim_start_matches(|c: char| !c.is_ascii_digit() && c != '.' && c != ',');
            if let Some(val) = parse_float_lenient(num_part) {
                return Some(val.round().clamp(0.0, 100.0) as u8);
            }
        } else if *word == "%" && i > 0 {
            if let Some(val) = parse_float_lenient(words[i - 1]) {
                return Some(val.round().clamp(0.0, 100.0) as u8);
            }
    ...
    // 2. Check for byte download ratio: e.g. "12,4 MB / 25,0 MB" or "12.4 MB / 25.0 MB"
    let lower_line = line.to_lowercase();
    let has_bytes = lower_line.contains("mb") || lower_line.contains("kb") || lower_line.contains("gb")
        || lower_line.contains("mib") || lower_line.contains("kib") || lower_line.contains("gib")
        || lower_line.contains("bytes") || lower_line.contains("b/s");
    ...
    if has_bytes {
        if let Some(slash_idx) = line.find('/') {
            ...
            let before_num = before.split_whitespace().rev().find_map(|w| parse_float_lenient(w));
            let after_num = after.split_whitespace().find_map(|w| parse_float_lenient(w));
...
fn parse_luppo_progress(line: &str, pkg_name: &str, action: &str) -> Option<ProgressEvent> {
    ...
    let lower = line.to_lowercase();
    let parsed_download = extract_percentage_or_ratio(line);

    let (progress, status) = if let Some(dl_pct) = parsed_download {
        if dl_pct >= 100 {
            (90, "installing")
        } else {
            // Scale download percentage strictly to 0..90%
            let scaled = ((dl_pct as f32 / 100.0) * 90.0).round().clamp(0.0, 90.0) as u8;
            (scaled, "downloading")
        }
    } else if lower.contains("tamamlandı") || lower.contains("complete") || lower.contains("başarılı") {
        (100, "completed")
    } else if lower.contains("kurul") || lower.contains("install") || lower.contains("yapılandır") || lower.contains("configur") || lower.contains("extract") || lower.contains("açılıyor") || lower.contains("unpack") {
        (90, "installing")
...
fn parse_flatpak_progress(line: &str, pkg_name: &str, action: &str) -> Option<ProgressEvent> {
    ...
    } else if lower.contains("complete") || lower.contains("tamamlandı") || lower.contains("success") {
        (100, "completed")
    } else if action == "remove" && (lower.contains("uninstall") || lower.contains("kaldır") || lower.contains("remov")) {
        (50, "removing")
    } else if lower.contains("deploying") || lower.contains("dağıtılıyor") {
        (90, "installing")
    } else {
        // Any other info line (commit hashes, "Installing...", "Updating...", metadata)
        // stays at 0% downloading, NEVER jumps to 90%!
        (0, "downloading")
    };
...
pub static RUNNING_PIDS: std::sync::LazyLock<std::sync::Mutex<HashMap<String, u32>>> =
    std::sync::LazyLock::new(|| std::sync::Mutex::new(HashMap::new()));

pub fn cancel_process(pkg_name: &str) -> bool {
    let pid_opt = RUNNING_PIDS.lock().unwrap().remove(pkg_name);
...
"#;
    ...
    #[test]
    fn test_download_progress_scaling() {
        // 1. Step counts like [1/4] must NOT jump progress artificially
        let step_event = parse_luppo_progress("[1/4] Paketler indiriliyor...", "test-pkg", "install");
        assert_eq!(step_event.unwrap().progress, 0); // generic downloading starts at 0, not jumping to arbitrary steps!

        // 2. Download percentage must scale up to 90%
        let dl_50 = parse_luppo_progress("İndiriliyor: %50 (12.5 MB / 25.0 MB)", "test-pkg", "install");
        assert_eq!(dl_50.unwrap().progress, 45); // 50% * 0.9 = 45%

        let dl_100 = parse_luppo_progress("İndiriliyor: %100 (25.0 MB / 25.0 MB)", "test-pkg", "install");
        assert_eq!(dl_100.unwrap().progress, 90); // 100% download = 90%

        // 3. Post-download installation / configuration holds at 90%
        let install_step = parse_luppo_progress("Paket kuruluyor ve yapılandırılıyor...", "test-pkg", "install");
        assert_eq!(install_step.unwrap().progress, 90);

        // 4. Completed reaches 100%
        let comp = parse_luppo_progress("İşlem başarıyla tamamlandı.", "test-pkg", "install");
        assert_eq!(comp.unwrap().progress, 100);

        // 5. Flatpak progress
        let fp_dl = parse_flatpak_progress("12.5 MB / 25.0 MB", "flatpak:org.test.App", "install");
        assert_eq!(fp_dl.unwrap().progress, 45);

        let fp_inst_announce = parse_flatpak_progress("Installing org.test.App", "flatpak:org.test.App", "install");
        assert_eq!(fp_inst_announce.unwrap().progress, 0); // initial announce does not jump to 90!

        let fp_commit = parse_flatpak_progress("Commit: 7a9df87b9c123456", "flatpak:org.test.App", "install");
        assert_eq!(fp_commit.unwrap().progress, 0); // commit hash line MUST NOT jump to 90!

        let fp_deploy = parse_flatpak_progress("Deploying org.test.App", "flatpak:org.test.App", "install");
        assert_eq!(fp_deploy.unwrap().progress, 90); // true post-download deploy phase is 90%

        let fp_dl_comma = parse_flatpak_progress("12,5 MB / 25,0 MB", "flatpak:org.test.App", "install");
        assert_eq!(fp_dl_comma.unwrap().progress, 45); // comma decimal support

        let fp_comp = parse_flatpak_progress("Installation complete", "flatpak:org.test.App", "install");
        assert_eq!(fp_comp.unwrap().progress, 100);
    }
}
